...
def lambda_handler(event, context):
    ...
    print (event)
    print (event.get('methodArn'))
    principalId = 'user:test'
    tmp = event['methodArn'].split(':')
    apiGatewayArnTmp = tmp[5].split('/')
    awsAccountId = tmp[4]
    apiKey = event.get('headers').get('X-API-KEY')
    policy = AuthPolicy(principalId, awsAccountId, apiKey)
    policy.restApiId = apiGatewayArnTmp[0]
    policy.region = tmp[3]
    policy.stage = apiGatewayArnTmp[1]
    try:
        Jwt.checkAccessToken(event.get('headers').get('auth'))
...
class AuthPolicy(object):
    ...
    def __init__(self, principal, awsAccountId, apiKey):
    ...
    def build(self):
        ...
        policy = {
            'usageIdentifierKey' : self.apiKey,
